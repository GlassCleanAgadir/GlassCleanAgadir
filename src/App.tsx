...
const EstimateForm = ({ onSuccess, lang }: { onSuccess?: () => void, lang: Language }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("https://formspree.io/f/xlgovaqw", {
        method: "POST",
        headers: {
      ...
      alert(lang === 'ar' ? "عذراً، حدث خطأ أثناء إرسال الطلب. يرجى المحاولة مرة أخرى." : "Sorry, an error occurred while sending your request. Please try again.");
    } finally {
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          >
            <div className="bg-blue-600 p-6 sm:p-8 text-white flex-shrink-0 relative">
              <button onClick={onClose} className="absolute top-6 right-6 text-white/80 hover:text-white z-20">
                <X size={24} />
              </button>
              <h2 className="text-2xl sm:text-3xl font-bold mb-2 pr-8">{t.title}</h2>
